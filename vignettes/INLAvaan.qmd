...
To motivate the fitting of SEMs, consider the introductory example in @song2012basic.
Answer the question of Does poorer glycemic control lead to greater severity of kidney disease?
We have three observed indicators of glycemic control (y1, y2, y3) and three observed indicators of kidney disease severity (y4, y5, y6).
Instead of fitting separate regression models for each indicator, we can use a SEM to model the relationships between the latent constructs of glycemic control and kidney disease severity.
...
"
set.seed(123)
dat <- lavaan::simulateData(pop_mod, sample.nobs = 500)
glimpse(dat)
```
