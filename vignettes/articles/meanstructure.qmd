...
estimated intercepts would. Models with exogenous covariates
(`fixed.x = TRUE`, the lavaan default --- including binary covariates,
whose moments stay frozen at their sample values and are never given a
Gaussian model) are fully supported: the mean marginalisation factorises
blockwise, and `loo()`/`waic()` score each unit by the difference of two
...
the *whole* measurement-invariance ladder; see the
[multigroup article](multigroup.html) for that workflow.
